...
{
    private const string MacHelperBundleName = "MyPowerTools Remote Notifications.app";
    private const string MacNativeLibraryName = "libMptMacNative.dylib";
    ...
    public static INotificationService? Create()
    {
        if (AllowTestBackend())
        {
            ...
            if (string.Equals(mode, "record", StringComparison.OrdinalIgnoreCase))
            {
                ...
                var recordingAuthorization = OperatingSystem.IsMacOS()
                    ? ValidateMacRecordingHost()
                    : "test-provider";
                SetSnapshot(new RemoteNotificationDeliverySnapshot(
                    "recording", recordingAuthorization, "", "", ""));
                return new RecordingNotificationService(
                    ExpandPath(recordPath),
    ...
    private static string ValidateMacRecordingHost()
    {
        var executableDirectory = new DirectoryInfo(
            Path.GetFullPath(AppContext.BaseDirectory)
                .TrimEnd(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar));
        var helperBundle = executableDirectory.Parent?.Parent;
        if (!string.Equals(executableDirectory.Name, "MacOS", StringComparison.Ordinal) ||
            !string.Equals(executableDirectory.Parent?.Name, "Contents", StringComparison.Ordinal) ||
            !string.Equals(helperBundle?.Name, MacHelperBundleName, StringComparison.Ordinal))
        {
            throw new InvalidOperationException(
                "The macOS recording backend requires the installed MyPowerTools Remote Notifications helper bundle.");
        }

        var nativeLibrary = Path.Combine(executableDirectory.FullName, MacNativeLibraryName);
        if (!File.Exists(nativeLibrary))
        {
            throw new InvalidOperationException(
                $"The macOS recording backend could not find the native notification library at '{nativeLibrary}'.");
        }

        var authorization = new MacUserNotificationService().GetAuthorizationStatus();
        if (authorization is not ("authorized" or "provisional" or "not-determined" or "denied"))
        {
            throw new InvalidOperationException(
                "The macOS recording backend could not initialize UserNotifications from the helper bundle.");
        }
    ...
    private sealed class RecordingNotificationService(
        string path,
        string authorization) : INotificationService
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            RecordSuccess(request.Id, authorization, "recorded");
