...
sealed class RemoteNotificationsLegacyStore : IRemoteNotificationsStore
{
     ...
     *  must remain visible. Items the v1 matcher wrongly relabeled are
     *  repaired back to their original human conversation body. */
    ...
    private static bool IsTaskTriggerBody(string body)
    {
        ...
        if (ClaudeTaskFingerprints.Any(fp => quotedBody.StartsWith(fp, StringComparison.Ordinal)))
        {
            return true;
        }
        var remainder = string.Join("\n", lines.Skip(index)).Trim();
        var candidate = (remainder.Length > 0 ? remainder : string.Join("\n", quoted)).TrimStart();
        if (candidate.StartsWith("[", StringComparison.Ordinal))
        {
            var close = candidate.IndexOf(']');
            if (close > 0)
            {
                candidate = candidate[(close + 1)..].TrimStart();
            }
        }
        return ClaudeTaskFingerprints.Any(fp => candidate.StartsWith(fp, StringComparison.Ordinal));
    }
    ...
    /// <summary>
    /// Removes legacy Claude Stop records that were generated by an automatic
    /// task/system turn while preserving legacy human conversations. Modern
    /// records carry source metadata and are left untouched; old records are
    /// classified only from the short synthetic body fingerprints.
    /// </summary>
        ...
        IReadOnlyList<RemoteNotificationRecord> messagesOldestFirst)
    {
        ...
        foreach (var message in CollapseClaudeStopDuplicates(messagesOldestFirst))
        {
            ...
            if (IsLegacyClaudeAutomaticReply(rewritten))
            {
                continue;
            }
        ...
        return MergeTaskCompletedRecords(cleaned);
    }
    ...
    private static bool IsLegacyClaudeAutomaticReply(RemoteNotificationRecord message)
    {
        ...
        if (HasSyntheticQuotedRequest(message.Message ?? ""))
        {
            return true;
        }

        var body = StripLeadingQuotedRequest(message.Message ?? "").Trim();
        if (string.Equals(ExtractLabel(body), ClaudeTaskLabel, StringComparison.Ordinal))
        {
            return true;
        }

        return IsTaskTriggerBody(body);
    }

    private static bool HasSyntheticQuotedRequest(string message)
    {
        ...
        foreach (var line in lines)
        {
            ...
            quoted.Add(line.TrimStart()[1..].TrimStart());
        }
        if (quoted.Count == 0)
        {
            return false;
        ...
        var quotedBody = string.Join("\n", quoted).TrimStart();
        return ClaudeTaskFingerprints.Any(fp => quotedBody.StartsWith(fp, StringComparison.Ordinal));
    }
