...
public sealed class RemoteNotificationDetailWindowService : IDisposable
{
    ...
    public RemoteNotificationDetailWindowService(
        IRemoteNotificationsStore? store = null, IMptWebSurfaceService? webSurfaces = null)
    {
    ...
    public bool CanOpen(string messageId) => TryFindRecord(_store.Load().MessagesOldestFirst, messageId, out _);
    ...
    public bool Open(RemoteNotificationMessageViewModel message)
    {
        ...
        {
            // A reused window may have navigated to another session message.
            existing.SetMessage(message);
            Present(existing);
            return true;
        }

        // Inject before SetMessage performs its first session lookup.
        var detail = new RemoteNotificationDetailWindow(message, _store, _webSurfaces);
    ...
    private static void BringToForeground(RemoteNotificationDetailWindow window)
    {
        ...
        if (foregroundSet && GetForegroundWindow() == handle) return;
    ...
    public static bool TryFindRecord(
        ...
        string messageId, out RemoteNotificationRecord record)
    {
