...
{
    private Task CopyFilteredTimelineAsync()
    {
        if (DataContext is not RemoteNotificationsViewModel vm || vm.VisibleMessages.Count == 0)
            return Task.CompletedTask;
        return CopyMessageAsync(RemoteNotificationTimelineFormatter.Format(vm.VisibleMessages, vm.FilterLabel, vm.SearchQuery));
    }
