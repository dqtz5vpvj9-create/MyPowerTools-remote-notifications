...
sealed class RemoteNotificationsLegacyStore : IRemoteNotificationsStore
{
    ...
    public RemoteNotificationsSnapshot Load()
    {
        ...
        var messages = MergeTaskCompletedRecords(
            CollapseClaudeStopDuplicates(originalMessages)
                .Where(message => !IsInternalAgentCommunication(message))
                .ToArray());
    ...
    public void SaveMessages(IReadOnlyList<RemoteNotificationRecord> messagesOldestFirst)
    {
        ...
        var collapsed = MergeTaskCompletedRecords(
            CollapseClaudeStopDuplicates(messagesOldestFirst)
                .Where(message => !IsInternalAgentCommunication(message))
                .ToArray());
