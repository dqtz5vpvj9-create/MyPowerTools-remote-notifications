...
# DSH Stop hook -> Remote Notifications queue (Windows deployment wrapper).
# The hooks-codex bridge runs this script through `pwsh -Command`; stdin is the
# snake_case Codex-shaped DSH payload. Enqueue is kept short and the worker is
# started detached so the hook never blocks the DSH turn boundary.

$ErrorActionPreference = 'Stop'
$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$logRoot = Join-Path $env:TEMP 'codex-hooks'
New-Item -ItemType Directory -Force -Path $logRoot | Out-Null
$logPath = Join-Path $logRoot 'dsh_hook.log'
$rawPath = Join-Path $logRoot 'dsh_hook_raw_stdin.json'
